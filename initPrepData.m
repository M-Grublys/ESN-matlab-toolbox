...
% U = 1*ones(1,2e4+1);   % y = 1
% U = 10*ones(1,2e4+1);  % y = 10
% t = dt.*(0:1:2e4);     % time
...
% tic
% [t, U] = genSystemRossler(dt, 2e4, [-9,0,0], [0.2,0.2,5.7]);
% toc
...
% tic
% [t, U] = genSystemLiu(dt, 2e4, [2.2,2.4,38], [10,40,1,2.5,4]); % Liu
% toc
% LyapunovTime = 0.6085;

%%
% S = size(U,1)*10;
% U2 = zeros(S,length(U)-S);
% for s = 1:length(U)-S
%     U2(:,s) = [U(1,s:s+S),U(2,s:s+S)];%,U(3,s:s+S)];
% end
% U3 = U;
% U = U2;
...
U = transform2window(U,0);  % window length L=0->[u(1)], L=1->[u(1),u(2)]
%                             % so L=L -> [u(1),...,u(L+1)]
...
% NEEDS MORE WORK IN FORECASTING SETUP:
% 
% windowLength = 3;
...
% Data = struct('name','Lorenz',...
%               'U', U,...
%               't', t,...
%               'dt', dt,...
%               'ic', [1,1,1],... [0,-0.01,9]
%               'otherParams', [10, 8/3, 28]);
