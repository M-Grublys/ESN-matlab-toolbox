...
ESN-matlab-toolbox is a MATLAB framework developed during my PhD research for constructing, training, forecasting with, and analysing Echo State Networks (ESNs). It supports custom ESN configurations, time-series forecasting, hyperparameter grid searches, batch training, and numerical analysis of reservoir dynamics.

The toolbox was developed for research into Reservoir Computing, chaotic time-series forecasting, and statistical properties of ESN activations.
...
* ESN is initialised, trained, and evaluated in `BasicScript.m`.
	...
	* Generate a forecast and evaluate the ESN's performance.
...
## Appendix Script
An example of ESN architecture can be found in `AppendixExampleScript.m`. The script can be run independently to train an ESN for forecasting a 2D circular trajectory. A copy of this code can be found in the Appendix C of my thesis.
